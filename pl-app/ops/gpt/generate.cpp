...
int main(int argc, char **argv) {
  ...
  int64_t past_rank = 4;
  int64_t past_shape[] = {64, 12, 256, 64};
  for (int i = 0; i < 12; ++ i) {
    ...
    inputTensors[i * 2 + 1] = omTensorCreateWithOwnership(pkey.data, past_shape, past_rank, ONNX_TYPE_FLOAT, true);
    inputTensors[i * 2 + 2] = omTensorCreateWithOwnership(pval.data, past_shape, past_rank, ONNX_TYPE_FLOAT, true);
  }
